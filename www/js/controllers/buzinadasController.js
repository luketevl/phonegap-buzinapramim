...
db.transaction(function(transaction){
	...
	$scope.mostrarBuzinadas = function(tx){
		tx.executeSql("SELECT * from toots", [], $scope.successShow, $scope.errorDB );
	};
    $scope.errorDB = function(err){
      console.log('erro' + err.code);
    };
		 $scope.successShow = function(tx, results){
			 $scope.loading = false;
			 $scope.buzinadas = JSON.parse(JSON.stringify(results.rows));
			 $scope.$apply();
			 console.log($scope.buzinadas);
		 };
		 $scope.successUpdate = function(tx, results){
			$('.checkbox-loader').css('display', 'none');
 			$('.label-switch').css('display', 'inline-block');
			db.transaction($scope.mostrarBuzinadas, $scope.errorDB);
		};
